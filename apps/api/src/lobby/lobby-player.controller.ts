...
export const addPlayer = async (
    ...
    try {
        ...
        const player = await PlayerServices.addLobbyPlayerService(
            lobbyId,
            req.user,
            playerData,
        );
...
export const updatePlayer = async (
    ...
    try {
        if (!req.user)
            return next(new AppError('Authentication required', 401));
        const { lobbyId, playerId } =
            req.validatedParams as LobbyPlayerParamInput;
        const playerData = req.validatedBody as UpdatePlayerInput;
        const player = await PlayerServices.updateLobbyPlayerService(
            lobbyId,
            playerId,
            req.user,
            playerData,
...
export const removePlayer = async (
    ...
    try {
        ...
        const { lobbyId, playerId } =
            req.validatedParams as LobbyPlayerParamInput;
        await PlayerServices.removeLobbyPlayerService(
            lobbyId,
            playerId,
            req.user,
        );
